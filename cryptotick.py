...
def getData(config):
    ...
    for x in range(0, num_retries):
        ...
        if connectfail == True:
            ...
            sleep_time *= (
                2  # Implement your backoff algorithm here i.e. exponential backoff
            )
        else:
            connectbool = False
            break
...
def updateDisplay(image, config, allprices, volumes):
    ...
    for key in allprices.keys():
        ...
        if config["ticker"]["exchange"] == "default":
            ...
            text = (
                pricechange + " vol:" + vol
            )  # Currency string omitted as gdax provides volume in coin number
    ...
    _place_text(
        image,
        "Updated: " + text + ". " + str(days_ago) + " day data",
        x_offset=-25,
        y_offset=-430,
        fontsize=50,
...
def main():
    ...
    if len(fiat_list) != len(curr_list):
        logging.info(
            "Fiat and Crypto lists differ in length. Using first fiat entry only"
        )
        fiat = fiat_list[0]
    ...
    try:
        while True:
            ...
            if (
                ...
                and config["display"]["maximalist"] != True
            ):
